				      ...
				      GtkSourceBuffer *buffer)
{
  ...
  if (gtk_source_buffer_iter_has_context_class (buffer, iter, "comment")
    ...
    while ((gtk_source_buffer_iter_has_context_class (buffer, iter, "comment")
      ...
      ;
  /* If the cursor is on a string, skip the string and the following
  ...
  /* Otherwise, the cursor is on other token.  Skip it and the
				       ...
				       GtkSourceBuffer *buffer)
{
  ...
  if (gtk_source_buffer_iter_has_context_class (buffer, iter, "comment")
    ...
    while ((gtk_source_buffer_iter_has_context_class (buffer, iter, "comment")
	   ...
	   && gtk_text_iter_backward_char (iter))
      ;
  /* If the cursor is on a string, skip the string and the preceding
     comments and whitespaces.  */
  else if (gtk_source_buffer_iter_has_context_class (buffer, iter, "string"))
    {
      while (gtk_source_buffer_iter_has_context_class (buffer, iter, "string")
  ...
  /* Otherwise, the cursor is on other token.  Skip it and the
     preceding comments and whitespaces.  */
  else
    {
      while (!g_unichar_isspace (gtk_text_iter_get_char (iter))
	     && gtk_text_iter_backward_char (iter))
